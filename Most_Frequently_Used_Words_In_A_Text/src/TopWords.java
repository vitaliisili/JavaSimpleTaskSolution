...
public class TopWords {
    public static void main(String[] args) {
        String str = "mxjtl''y!ad' 'asd asdas'asd   !sadas'=-ad'!"; // [ad', mxjtl''y, 'asd]
    ...
    public static List<String> top3(String s) {
        ...
        String[] arr = s.toLowerCase().replaceAll("[^a-z'\s]", " ").trim().split("\s+");
        for (String word : arr) {
            if (unsorted.containsKey(word)) {
                unsorted.put(word, unsorted.get(word) + 1);
            } else if(word.matches("\\b([a-z]|')+|([a-z]|')+\\b")){
                unsorted.put(word, 1);
            }
        }

        List<String> result = unsorted.entrySet()
                .stream()
